...
foreign import _reverseObj :: Json -> Json
...
instance DecodeJson Personal where
  decodeJson json = do
    ...
    height <- (decodeJson =<< obj .: "height") :: Either _ Centimeters
    weight <- (decodeJson =<< obj .: "weight") :: Either _ Kilograms
    age <- (decodeJson =<< obj .: "age") :: Either _ Years
...
test = launchAff_ do
  ...
  log case bodies of
    ...
    Right [ teacherJson, studentJson ] ->
      show (decode teacherJson :: _ Teacher) <> "\n\n"
        <> show (decode studentJson :: _ Student)
    Right _ -> "Wrong Array size"
  where
  decode :: ∀ a. DecodeJson a => Json -> Either JsonDecodeError a
  decode = decodeJson <<< _reverseObj

  postRequest :: ∀ a. EncodeJson a => a -> Aff (Either Error (Response Json))
  postRequest = AjaxNode.post ResponseFormat.json "http://localhost:3000/"
    <<< Just
    <<< RequestBody.String
    <<< ArgCore.stringify
    <<< encodeJson
