...
import CSS.Flexbox (alignItems, flexBasis, flexDirection, flexEnd, flexGrow, flexShrink, flexStart, justifyContent, row)
import CSS.Font (FontWeight(..), color, fontSize, fontWeight)
import CSS.Geometry (height, minHeight, padding, paddingLeft, paddingRight, paddingTop, width)
...
component style innerComponent = H.mkComponent
  ...
  where
  handleAction :: Action iInput iOutput -> H.HalogenM (State iInput) (Action iInput iOutput) (Slots iQuery iOutput) iOutput m Unit
  handleAction = case _ of
    Input input -> H.modify_ _ { iInput = input }
    Output output -> H.raise output
    Logoff -> navigate Route.Logoff

  handleQuery :: ∀ a. iQuery a -> H.HalogenM (State iInput) (Action iInput iOutput) (Slots iQuery iOutput) iOutput m (Maybe a)
